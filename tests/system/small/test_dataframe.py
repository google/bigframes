...
def test_insert(scalars_dfs, loc, column, value, allow_duplicates):
    ...
    bigframes.testing.utils.assert_frame_equal(
        bf_df.to_pandas(), pd_df, check_dtype=False
    )
...
def test_drop_labels_axis_1(scalars_dfs):
    ...
    bigframes.testing.utils.assert_frame_equal(pd_result, bf_result)
...
def test_df_memory_usage(scalars_dfs):
    ...
    bigframes.testing.utils.assert_series_equal(pd_result, bf_result, rtol=1.5)
...
def test_df_peek(scalars_dfs_maybe_ordered):
    ...
    bigframes.testing.utils.assert_index_equal(
        scalars_pandas_df.columns, peek_result.columns
    )
...
def test_get_df_column_name_duplicate(scalars_dfs):
    ...
    bigframes.testing.utils.assert_index_equal(bf_result.columns, pd_result.columns)
...
def test_df_to_pandas_batches(scalars_dfs):
    ...
    assert_frame_equal(pd.concat(filtered_batches), pd_result)
...
def test_assign_new_column_w_setitem_dataframe(scalars_dfs):
    ...
    bigframes.testing.utils.assert_frame_equal(bf_df.to_pandas(), pd_df)
...
def test_df_cross_merge(scalars_dfs):
    ...
    bigframes.testing.utils.assert_frame_equal(
        bf_result, pd_result, check_index_type=False
    )
...
def test_df_update(overwrite, filter_func):
    ...
    bigframes.testing.utils.assert_frame_equal(bf_df1.to_pandas(), pd_df1)
...
def test_df_align(join, axis):
    ...
    bigframes.testing.utils.assert_frame_equal(
        bf_result1.to_pandas(), pd_result1, check_dtype=False
    )
    bigframes.testing.utils.assert_frame_equal(
        bf_result2.to_pandas(), pd_result2, check_dtype=False
    )
...
def test_join_different_table_with_duplicate_column_name(
    ...
    bigframes.testing.utils.assert_index_equal(bf_df_a.columns, pd_df_a.columns)
    bigframes.testing.utils.assert_index_equal(bf_df_b.index.to_pandas(), pd_df_b.index)
    bigframes.testing.utils.assert_frame_equal(
        bf_result, pd_result, check_index_type=False
    )
...
def test_dataframe_numeric_analytic_op(
    ...
    bigframes.testing.utils.assert_frame_equal(pd_series, bf_result, check_dtype=False)
...
def test_df_transpose():
    ...
    assert_frame_equal(pd_result, bf_result, check_dtype=False, nulls_are_nan=True)  # type: ignore
...
def test_dataframe_bool_aggregates(scalars_df_index, scalars_pandas_df_index, op):
    ...
    bigframes.testing.utils.assert_series_equal(
        pd_series, bf_result, check_index_type=False
    )
...
def test_df_skew_too_few_values(scalars_dfs):
    ...
    bigframes.testing.utils.assert_series_equal(
        pd_result, bf_result, check_index_type=False
    )
...
def test_df_duplicated(scalars_df_index, scalars_pandas_df_index, keep, subset):
    ...
    bigframes.testing.utils.assert_series_equal(pd_series, bf_series, check_dtype=False)
...
def test_df_value_counts(scalars_dfs, subset, normalize, ascending, dropna):
    ...
    bigframes.testing.utils.assert_series_equal(
        ...
        check_dtype=False,
        check_index_type=False,
        ignore_order=True,  # different pandas versions inconsistent for tie-handling
    )
...
def test_resample_with_column(
    ...
    if pandas.__version__.startswith("3"):
        pytest.skip(
            "pandas 3.0 behavior diverges for day offsets: https://github.com/pandas-dev/pandas/pull/61985"
        )
    bf_result = (
...
def test_resample_with_index(
    ...
    # TODO: supply a reason why this isn't compatible with pandas 1.x
    if rule == "100d" and pandas.__version__.startswith("3"):
        pytest.skip(
            "pandas 3.0 behavior diverges for day offsets: https://github.com/pandas-dev/pandas/pull/61985"
        )
    pytest.importorskip("pandas", minversion="2.0.0")
