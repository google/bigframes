...
def test_read_pandas_multi_index_axes():
    ...
    bigframes.testing.utils.assert_frame_equal(bf_df_computed, pandas_df)
...
def test_multi_index_dataframe_join(scalars_dfs, how):
    ...
    bigframes.testing.utils.assert_frame_equal(bf_result, pd_result, ignore_order=True)
...
def test_column_multi_index_getitem(scalars_df_index, scalars_pandas_df_index):
    ...
    pd_a = pd_df["a"]
    bigframes.testing.utils.assert_frame_equal(bf_a, pd_a)

    bf_b = bf_df["b"].to_pandas()
    pd_b = pd_df["b"]
    bigframes.testing.utils.assert_frame_equal(bf_b, pd_b)

    bf_fullkey = bf_df[("a", "int64_too")].to_pandas()
    pd_fullkey = pd_df[("a", "int64_too")]
    bigframes.testing.utils.assert_series_equal(bf_fullkey, pd_fullkey)
