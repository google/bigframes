...
def test_series_construct_nullable_ints():
    ...
    bigframes.testing.utils.assert_series_equal(bf_result, expected)
...
def test_series_construct_pandas(scalars_dfs):
    ...
    bigframes.testing.utils.assert_series_equal(bf_result.to_pandas(), pd_result)
...
def test_series_construct_w_dtype(dtype):
    ...
    bigframes.testing.utils.assert_series_equal(series.to_pandas(), expected)
...
def test_series_construct_local_unordered_has_sequential_index(unordered_session):
    ...
    bigframes.testing.utils.assert_index_equal(series.index.to_pandas(), expected)
...
def test_series_construct_w_nested_json_dtype():
    ...
    if hasattr(pa, "JsonType"):
        ...
        bigframes.testing.utils.assert_series_equal(s.to_pandas(), s2.to_pandas())
...
def test_series_keys(scalars_dfs):
    ...
    bigframes.testing.utils.assert_index_equal(bf_result, pd_result)
...
def test_series___setitem__(scalars_dfs, index_col, key, value):
    ...
    bigframes.testing.utils.assert_series_equal(bf_series.to_pandas(), pd_series)
...
def test_fillna_inplace(scalars_dfs, col_name, fill_value):
    scalars_df, scalars_pandas_df = scalars_dfs
...
def test_reset_index_no_drop(scalars_df_index, scalars_pandas_df_index, name):
    ...
    bigframes.testing.utils.assert_frame_equal(bf_result.to_pandas(), pd_result)
...
def test_copy(scalars_df_index, scalars_pandas_df_index):
    ...
    bigframes.testing.utils.assert_series_equal(bf_copy.to_pandas(), pd_copy)
...
def test_loc_setitem_cell(scalars_df_index, scalars_pandas_df_index):
    ...
    bigframes.testing.utils.assert_series_equal(bf_original.to_pandas(), pd_original)
...
def test_series_astype_python(session):
    ...
    bigframes.testing.utils.assert_series_equal(result, exepcted)
...
def test_date_time_astype_int(
    ...
    bf_result = scalars_df_index[column].astype(to_type).to_pandas()
    pd_result = scalars_pandas_df_index[column].astype(to_type)
    bigframes.testing.utils.assert_series_equal(bf_result, pd_result, check_dtype=False)
...
def test_string_astype_json_in_safe_mode(session):
    ...
    expected.index = expected.index.astype("Int64")
    bigframes.testing.utils.assert_series_equal(bf_result.to_pandas(), expected)
